...
flags.DEFINE_integer('num_filters', 20, 'number of filters for conv nets -- 32 for miniimagenet, 64 for omiglot.')
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr_now in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if (itr_now != 0) and itr_now % TEST_PRINT_INTERVAL == 0:
            ...
            # '''
            # result是np.ndarray
            # shape of inputa (4, 5*1 , 640)
            # shape of inputb (4, 5*15, 640)
            # shape of labela (4, 5*1 , 5)
            # shape of labelb (4, 5*15, 5)
            # '''
            # print('当前用于训练的数据：')
            # for i in range(2):
            #     print('inputa:{}'.format(result[0][0][i][:10]))
            #     print('labela:{}'.format(result[2][0][i]))
            # result = sess.run(model.weights)
            # print('当前 weight[w1]')
            # for i in range(3):
            #     print(result['w1'][i][:4])
            # print(result['w1'][5])
...
def _test(model, saver, sess, exp_string, data_generator, test_num_updates=None):
    ...
    # 循环结束
    print('acc1:{}'.format(result_acc1))
    print('acc2:{}'.format(result_acc2))
    print('loss1:{}'.format(result_loss_1))
    print('losses2:{}'.format(result_loss_2))
