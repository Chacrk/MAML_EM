...
class MAML:
    def __init__(self, dim_input=1, dim_output=1, test_num_updates=5):
        ...
        self.metatrain_op_name = 'adam'
        self.global_step = 0
        self.opt_switch_position = FLAGS.opt_switch_position
        self.lr_switch_position = FLAGS.lr_switch_position
    ...
    def construct_model(self, input_tensors, prefix='metatrain_'):
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(input_, reuse=True):
                # 执行这里的时候，inputa为 (1*5, 640)，inputb为 (15*5, 640)，为其中一个task，map_fn的效果
            ...
            '''
            out_dtype包含：
            [task_outputa, task_outputbs, task_lossa, task_lossesb, task_accuracy_a, task_accuracies_b]
            '''
            out_dtype = [tf.float32, [tf.float32] * num_updates, tf.float32, [tf.float32] * num_updates]
            out_dtype.extend([tf.float32, [tf.float32] * num_updates])
            '''
            接收一个对象，对这个对象进行迭代处理，Parallel_iterations为batch_size
            '''
            ...
            '''
            lossesa和lossesb每次会更新，lossesa和lossesb代表的是w在经过4个task的更新后在最后一个task上的loss，也就是可以
            看作在所有tasks上loss效果的综合
            self.lossesa = lossesa # (4, 1*5)
            self.lossesb = lossesb # (5, 4, 15*5)
            tf.variable_scope结束
            '''

        # 判断是在训练还是测试
        if 'train' in prefix:
            # meta_batch_size :每个 batch含有的 task数量，losses已经除以 K了
            ...
            self.learning_rate = tf.cond(tf.greater(self.global_step, self.lr_switch_position),
                                         lambda: tf.train.exponential_decay(learning_rate=self.meta_lr * 0.1,
                                                                            global_step=self.global_step,
                                                                            decay_steps=self.decay_steps,
                                                                            decay_rate=self.decay_rate,
                                                                            staircase=False),
                                         lambda: tf.train.exponential_decay(learning_rate=self.meta_lr,
                                                                            global_step=self.global_step,
                                                                            decay_steps=self.decay_steps,
                                                                            decay_rate=self.decay_rate,
                                                                            staircase=False))
            ...
            self.gvs = sgd_optimizer.compute_gradients(loss_final)
            # 对SGD进行梯度修剪，
            self.gvs = [(tf.clip_by_value(grad, -1.0, 1.0), var) for grad, var in self.gvs]
            metatrain_op_sgd = sgd_optimizer.apply_gradients(self.gvs, global_step=self.global_step)
            ...
            self.metaval_total_accuracies2 \
                = total_accuracies2 = [tf.reduce_sum(accuraciesb[j]) / tf.to_float(FLAGS.meta_batch_size)
                                                                  for j in range(num_updates)]
    ...
    def construct_fc_weights(self, prefix):
        ...
        for i in range(1, len(self.dim_hidden)):
            ...
            weights['w'+str(i+1)] = tf.Variable(tf.truncated_normal([self.dim_hidden[i-1],
        ...
        # weights['w3']为输出层的
        weights['w'+str(len(self.dim_hidden)+1)] = tf.Variable(tf.truncated_normal([self.dim_hidden[-1],
